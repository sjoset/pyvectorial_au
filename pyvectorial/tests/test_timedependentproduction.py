...
def q_t_gaussian(vmc_gaussian):
    return TimeDependentProduction(
        vmc_gaussian.production.time_variation_type
    ).create_production(**vmc_gaussian.production.params)
...
def q_t_sine(vmc_sine):
    return TimeDependentProduction(
        vmc_sine.production.time_variation_type
    ).create_production(**vmc_sine.production.params)
...
def q_t_square_pulse(vmc_square_pulse):
    return TimeDependentProduction(
        vmc_square_pulse.production.time_variation_type
    ).create_production(**vmc_square_pulse.production.params)


# binned tests
def test_binned_q_first(vmc_binned):
    assert np.isclose(vmc_binned.production.params["q_t"][0], 7.0e25 / u.s)


def test_binned_q_last(vmc_binned):
    assert np.isclose(vmc_binned.production.params["q_t"][-1], 1.0e28 / u.s)


def test_binned_times_first(vmc_binned):
    assert np.isclose(
        vmc_binned.production.params["times_at_productions"][0], 35 * u.day  # type: ignore
    )


def test_binned_times_last(vmc_binned):
    assert np.isclose(
        vmc_binned.production.params["times_at_productions"][-1], 5 * u.day  # type: ignore
    )


# gaussian tests
def test_gaussian_amplitude(vmc_gaussian):
    assert np.isclose(vmc_gaussian.production.params["amplitude"], 3.0e28 / u.s)


def test_gaussian_std_dev(vmc_gaussian):
    assert np.isclose(vmc_gaussian.production.params["std_dev"], 20.0 * u.hour)  # type: ignore


def test_gaussian_t_max(vmc_gaussian):
    assert np.isclose(vmc_gaussian.production.params["t_max"], 24.0 * u.hour)  # type: ignore
...
def test_sine_amplitude(vmc_sine):
    assert np.isclose(vmc_sine.production.params["amplitude"], 3.0e28 / u.s)


def test_sine_period(vmc_sine):
    assert np.isclose(vmc_sine.production.params["period"], 20.0 * u.hour)  # type: ignore


def test_sine_delta(vmc_sine):
    assert np.isclose(vmc_sine.production.params["delta"], 3.0 * u.hour)  # type: ignore
...
def test_square_pulse_amplitude(vmc_square_pulse):
    assert np.isclose(vmc_square_pulse.production.params["amplitude"], 4.0e28 / u.s)


def test_square_pulse_duration(vmc_square_pulse):
    assert np.isclose(vmc_square_pulse.production.params["duration"], 5.0 * u.hour)  # type: ignore


def test_square_pulse_t_start(vmc_square_pulse):
    assert np.isclose(vmc_square_pulse.production.params["t_start"], 10.0 * u.hour)  # type: ignore
